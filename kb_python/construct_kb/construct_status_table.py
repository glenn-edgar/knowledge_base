...
class Construct_Status_Table:
 ...
 # Execute the SQL script to set up the schema
    ...
    def add_status_field(self, status_key, initial_properties, initial_data):
        """
        ...
        if not isinstance(initial_properties, dict):
            raise TypeError("initial_properties must be a dictionary")
        if not isinstance(initial_data, dict):
            raise TypeError("initial_data must be a dictionary")
            
        # Convert dictionaries to JSON strings
        properties_json = json.dumps(initial_properties)
