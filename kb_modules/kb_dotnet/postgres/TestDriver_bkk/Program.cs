    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"description: {kb.FindDescription(specificNode)}");
                
                var data = await kb.GetStatusData(spPath);
                Console.WriteLine($"initial data: {data.Data}");
                
                await kb.SetStatusData(spPath, new Dictionary<string, object> 
                { 
                    ["prop1"] = "val1", 
                    ["prop2"] = "val2" 
                });
                ...
                await kb.PushJobData(jobPath, new Dictionary<string, object> 
                { 
                    ["prop1"] = "val1", 
                    ["prop2"] = "val2" 
                });
                ...
                if (job != null)
                {
                    ...
                    await kb.MarkJobCompleted(Convert.ToInt32(jobDict["id"]), 3, 1000);
                }
                ...
                var descriptions = await kb.FindDescriptionPaths(streamKeys);
                Console.WriteLine($"descriptions: {descriptions.Count} found");
                
                if (streamKeys.Count > 0)
                {
                    await kb.ClearStreamData(streamKeys[0], null);
                    await kb.PushStreamData(streamKeys[0], new Dictionary<string, object> 
                    { 
                        ["prop1"] = "val1", 
                        ["prop2"] = "val2" 
                    }, 3, 1000);
        ...
        static async Task TestServerFunctions(KBDataStructures kb, string serverPath)
        {
            ...
            await kb.RpcServerClearServerQueue(serverPath, 3, 1000);
            ...
            foreach (var job in jobs)
            {
                if (job != null)
                {
                    ...
                    {
                        await kb.RpcServerMarkJobCompletion(serverPath, Convert.ToInt32(jobDict["id"]), 5, 1000);
        ...
        static async Task TestClientQueue(KBDataStructures kb, string clientPath)
        {
            ...
            await kb.RpcClientClearReplyQueue(clientPath, 3, 1000);
            Console.WriteLine($"cleared, free_slots: {await kb.RpcClientFindFreeSlots(clientPath)}");
            ...
            await kb.RpcClientClearReplyQueue(clientPath, 3, 1000);
            Console.WriteLine($"final free_slots: {await kb.RpcClientFindFreeSlots(clientPath)}");
        }
