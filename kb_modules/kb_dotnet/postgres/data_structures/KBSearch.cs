...
public class KBSearch
{
    ...
    {
        Filters.Add(new Filter { Condition = "path <@ #", Params = new List<object> { startingPath } });
    }

    public void SearchPath(string pathExpr)
    {
        Filters.Add(new Filter { Condition = "path ~ #", Params = new List<object> { pathExpr } });
    }
    ...
    public async Task<List<dynamic>> ExecuteQueryAsync()
    {
        ...
        try
        {
            ...
            {
                cmd.Parameters.AddWithValue($"${i + 1}", combinedParams[i]);
            }
    ...
    public async Task<Dictionary<string, object>> FindDescriptionPathsAsync(object pathArray)
    {
        ...
        if (paths.Count == 1)
        {
            query = $"SELECT path, data FROM {BaseTable} WHERE path = $1";
        }
        else
        {
            var placeholders = string.Join(", ", paths.Select((_, i) => $"${i + 1}"));
        ...
        try
        {
            ...
            {
                cmd.Parameters.AddWithValue($"${i + 1}", paths[i]);
            }
            ...
            while (await reader.ReadAsync())
            {
                ...
                var data = reader.IsDBNull("data") ? null : reader["data"];
                output[path] = data;
            }
