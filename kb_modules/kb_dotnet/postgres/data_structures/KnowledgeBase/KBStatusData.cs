...
public class KBStatusData
{
    ...
    public KBStatusData(KBSearch kbSearch, string database)
    {
        ...
        // Don't get connection here - get it when needed
    }
    ...
    public async Task<(object Data, string Path)> GetStatusDataAsync(string path)
    {
        ...
        var client = GetConnection(); // Get connection when needed
        var query = $@"
            SELECT data, path
            FROM {_baseTable}
            WHERE path = $1::ltree
    ...
    public async Task<Dictionary<string, object>> GetMultipleStatusDataAsync(object paths)
    {
        ...
        var client = GetConnection(); // Get connection when needed
        var placeholders = string.Join(", ", pathList.Select((_, i) => $"${i + 1}::ltree"));
        var query = $@"
            SELECT data, path
            FROM {_baseTable}
        ...
        {
            cmd.Parameters.AddWithValue(pathList[i]);
        }
...
";
        ...
        for (int attempt = 0; attempt <= retryCount; attempt++)
        {
            try
            {
                using var cmd = new NpgsqlCommand(upsertQuery, client);
                cmd.Parameters.AddWithValue(path);
                ...
                var returnedPath = reader["path"]?.ToString() ?? string.Empty;
