...
const KBDataStructures_1 = require("./KBDataStructures");
const uuid_1 = require("uuid");
const prompt = (0, prompt_sync_1.default)();
async function testServerFunctions(kb, serverPath) {
    ...
    const requestIds = [(0, uuid_1.v4)(), (0, uuid_1.v4)(), (0, uuid_1.v4)()];
    for (let i = 0; i < requestIds.length; i++) {
        await kb.rpcServerPushRpcQueue(serverPath, requestIds[i], `rpc_action${i + 1}`, { [`data${i + 1}`]: `data${i + 1}` }, `transaction_tag_${i + 1}`, i + 1, 'rpc_client_queue', 5, 500);
    }
...
async function testClientQueue(kb, clientPath) {
    console.log('=== Initial State ===');
...
async function main() {
    ...
    const kb = new KBDataStructures_1.KBDataStructures('localhost', 5432, 'knowledge_base', 'gedgar', password, 'knowledge_base');
    // === STATUS DATA TEST ===
    console.log('\n\n\n***************************  status data ***************************');
    ...
    const specificNode = await kb.findStatusNodeId('kb1', 'info2_status', { prop3: 'val3' }, '*.header1_link.header1_name.KB_STATUS_FIELD.info2_status');
    ...
    console.log('has_link_mount', await kb.executeKbSearch());
