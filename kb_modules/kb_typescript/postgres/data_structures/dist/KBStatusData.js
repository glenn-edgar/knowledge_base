...
class KBStatusData {
    ...
    async findNodeId(kb, nodeName, properties, nodePath) {
        const results = await this.findNodeIds(kb, nodeName, properties, nodePath);
        if (results.length === 0) {
            throw new Error(`No node found matching parameters: kb=${kb}, name=${nodeName}, properties=${JSON.stringify(properties)}, path=${nodePath}`);
        }
        if (results.length > 1) {
            throw new Error(`Multiple nodes (${results.length}) found matching parameters: kb=${kb}, name=${nodeName}, properties=${JSON.stringify(properties)}, path=${nodePath}`);
        }
        return results[0];
    }
    async findNodeIds(kb, nodeName, properties, nodePath) {
        try {
            ...
            if (!nodeIds || nodeIds.length === 0) {
                throw new Error(`No nodes found matching parameters: kb=${kb}, name=${nodeName}, properties=${JSON.stringify(properties)}, path=${nodePath}`);
            }
            return nodeIds;
        }
        catch (e) {
            if (e.message.startsWith('No nodes found'))
                throw e;
            throw new Error(`Error finding node IDs: ${e.message}`);
        }
    }
    async getStatusData(path) {
        if (!path)
    ...
    async getMultipleStatusData(paths) {
    ...
    async setStatusData(path, data, retryCount = 3, retryDelay = 1000) {
        if (!path)
            throw new Error('Path cannot be empty or None');
    ...
    `;
        ...
        throw new Error(`Failed to set status data for path '${path}' after ${retryCount + 1} attempts: ${lastError.message}`);
    }
    async setMultipleStatusData(pathDataPairs, retryCount = 3, retryDelay = 1000) {
        const pairs = Array.isArray(pathDataPairs)
            ? pathDataPairs
            : Object.entries(pathDataPairs);
        if (pairs.length === 0)
            throw new Error('path_data_pairs cannot be empty');
        if (retryCount < 0)
            throw new Error('Retry count must be non-negative');
        if (retryDelay < 0)
            throw new Error('Retry delay must be non-negative');
        const jsonPairs = pairs.map(([p, d]) => [p, JSON.stringify(d)]);
        const upsertQuery = `
      INSERT INTO ${this.baseTable} (path, data)
      VALUES ($1, $2)
      ON CONFLICT (path)
    ...
    `;
        ...
        throw new Error(`Failed to set multiple status data after ${retryCount + 1} attempts: ${lastError.message}`);
    }
}
exports.KBStatusData = KBStatusData;
