...
class KBRpcClient {
    ...
    async findRpcClientId(kb, nodeName, properties, nodePath) {
        const ids = await this.findRpcClientIds(kb, nodeName, properties, nodePath);
        if (ids.length === 0)
            throw new Error(`No node found matching parameters: ${nodeName}, ${JSON.stringify(properties)}, ${nodePath}`);
        if (ids.length > 1)
            throw new Error(`Multiple nodes found matching parameters: ${nodeName}, ${JSON.stringify(properties)}, ${nodePath}`);
        return ids[0];
    }
    async findRpcClientIds(kb, nodeName, properties, nodePath) {
    ...
    findRpcClientKeys(keyData) {
        return keyData.map(r => r.path);
    }
    async findFreeSlots(clientPath) {
        const query = `
    ...
    `;
        try {
            ...
            if (total === 0)
                throw new Error(`No records found for client_path: ${clientPath}`);
            return free;
        }
        catch (err) {
    ...
    async findQueuedSlots(clientPath) {
        const query = `
      SELECT COUNT(*) AS total_records,
    ...
    async peakAndClaimReplyData(clientPath, maxRetries = 3, retryDelay = 1000) {
        let attempt = 0;
        ...
        throw new Error(`Could not lock a new-reply row after ${maxRetries} attempts`);
    }
    async clearReplyQueue(clientPath, maxRetries = 3, retryDelay = 1000) {
        let attempt = 0;
        const table = this.baseTable;
        while (attempt < maxRetries) {
            try {
    ...
    async pushAndClaimReplyData(clientPath, requestUuid, serverPath, rpcAction, transactionTag, replyData, maxRetries = 3, retryDelay = 1000) {
        let attempt = 0;
        const table = this.baseTable;
        while (attempt <= maxRetries) {
            try {
    ...
    async listWaitingJobs(clientPath) {
        const table = this.baseTable;
        let query = `
...
exports.KBRpcClient = KBRpcClient;
